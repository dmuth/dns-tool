...
do
	...
	#CMD="./dns-tool.py -q --query-type ${TYPE} --graph ${QUESTION} ${SERVER}"
	CMD_OUT="./dns-tool.py -q --query-type ${TYPE} --raw --graph ${QUESTION} ${SERVER} "
	CMD_IN="./dns-tool.py --text --stdin google.com"
	echo "Running '$CMD_OUT | $CMD_IN'..." >&2
	$CMD_OUT | $CMD_IN
